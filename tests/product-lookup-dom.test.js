...
assert.equal((html.match(/<b>Product basis and limitations<\/b>/g) || []).length, 3);
...
assert.match(script, /EzyStrut Heavy Duty U Bolt datasheet/);
assert.match(script, /assets\/Datasheets\/Heavy-Duty-U-Bolt-DataSheet\.pdf/);
assert.match(script, /const productSourceCheckedDates = \{/);
assert.match(script, /function productSourceCheckedDate\(product\)/);
assert.match(script, /No curated catalogue entry/);
assert.match(script, /loads: \[9\.8, 14\.6\]/);
assert.match(script, /loads: \[174\.5, 224\.0\]/);
assert.match(script, /gripSourceConflict/);
assert.match(script, /torqueSourceConflict/);
assert.match(html, /app\.js\?v=20260818sectiondirectory2/);
