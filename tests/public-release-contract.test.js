...
assert.match(html, /Build 0\.7\.79/, "The visible public build must match this release");
assert.match(html, /styles\.css\?v=20260828padstm1/, "The current shared layout stylesheet must be cache-busted");
...
assert.match(html, /concrete-section-calculation\.js\?v=20260828padstm1/, "The current Concrete section module must be cache-busted");
...
assert.match(html, /app\.js\?v=20260828padstm1/, "The current shared display path must be cache-busted");
...
assert.match(styles, /\.member-selected-section \{[^}]*grid-template-columns: minmax\(0, 1fr\) 180px;/);
assert.match(styles, /\.member-summary-visual \{[^}]*max-width: 180px;/);
assert.match(styles, /\.member-summary-visual \.section-diagram \{[^}]*height: 104px;[^}]*min-height: 0;[^}]*max-height: 104px;[^}]*object-fit: contain;/);
...
assert.match(app, /selectedMember\.dataset\.memberGuide = type/);
assert.doesNotMatch(styles, /@media \(min-width: 1101px\) and \(max-width: 1320px\) \{\s*\.member-summary-primary \{[^}]*repeat\(3/);
