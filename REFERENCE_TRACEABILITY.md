...
## 2026-08-21 Build 0.7.74 Safety and Audit Integration

Build 0.7.74 integrates the Build 0.7.73 Monopole segment model, the current Beam, Weld and Axial Member releases, and the Concrete Pad shear-input safety correction. It does not change the accepted AS 3600 one-way shear equations. When vertical shear fitments are selected, the page requires a positive whole-number leg count, spacing of at least 50 mm and yield strength from 200 MPa to 600 MPa. Blank, zero, negative, non-numeric or out-of-range values are marked invalid and suppress both flexural and shear capacity.

`concrete-section-calculation.js` independently rejects non-positive or non-finite bar area, leg count, spacing and yield strength on the vertical-fitment path. It no longer replaces missing spacing with 1 mm or silently caps an invalid yield-stress input. The no-shear-reinforcement path remains independent of the disabled fitment controls. Regression coverage includes blank, zero, negative, non-numeric, fractional and out-of-range fitment inputs.

The Monopole bend-radius note now displays the live editable `r_i/t_nom` ratio rather than a hard-coded initial value. The README describes only the accepted `Profile segment schedule`. Nominal slip fit remains a necessary endpoint nesting screen; it does not verify acceptable clearance, tolerances, ovalisation, seam geometry, jacking force or joint resistance.

Independent Austube DCT 2013 reconciliation found the repository catalogue rows and calculated hollow-section results consistent with the published tables within printed rounding, except the published `150 x 50 x 2.0 RHS C450L0` shear value. The repository applies the DCT Section 5.2.2.4 slender-web reduction because `d_1/t sqrt(f_y/250) = 97.9 > 82`, giving `phi V_v = 92.5 kN`; the published 132 kN value corresponds to omitting that reduction. Record this as an apparent published-table inconsistency, not a confirmed manufacturer erratum.
