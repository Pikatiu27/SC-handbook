...
- Screw Piles Selector currently embeds a geometry-first selector using local Katana references plus Ideal Foundations, Blade Pile, Piletech, Driven Engineering, Keller, Minmetals Helicast, Surefoot, StopDigging, Ground Screws Australia and Helical Piles Australia source prompts. It is not a complete AS 2159 design engine. The UI distinguishes direction-specific product data, compression/system SWL-up-to references, indicative system ratings, typical benchmarks and geometry-only rows. Only direction-specific product/project values produce a normal comparison; SWL-up-to values are reference-only, while indicative ratings and typical benchmarks are excluded. The Katana Product Statement and Capacity Calculation Worksheet referenced in the CodeMark certificate are not present in the current Reference index. Non-local supplier documents remain pending for local-pack traceability until added and rebuilt.
