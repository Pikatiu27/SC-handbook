...
- Do not classify common engineering parameter names, conventional symbols, units, general concepts or independently stated factual information as source extracts merely because the same terms appear in a Standard or handbook. Copyright review applies to the reproduced expression, substantial selection or arrangement, source-specific wording, figures and table presentation; technical citation and verification remain separate requirements.
...
- identify candidate source extracts, figures, copied table structures, close paraphrases and substantial source-specific selections, then confirm whether citation alone is sufficient or reproduction permission is required; do not treat parameter names, symbols, units or general engineering facts alone as extracts;
...
- keep unresolved substantial-extract, copied-expression or source-entitlement items recorded as public-release blockers rather than implying that technical verification grants publication rights.
...
- `Ground Parameters` (`Unreleased` local reference aid)
...
| `Ground Parameters` | `Foundations` | Controlled ground-description lookup with classification, physical/state, strength, deformation, hydraulic/durability and foundation/interface groups | AS 1726 classifications checked; interpretive handbook ranges For Review; project-report-only rows fail closed | `Unreleased` local main-app tab; stripped from the public artifact |
...
- `Foundations`: the `Unreleased` local-only `Ground Parameters` tab, followed by `Concrete Pad Section`, `Reinforcement`, `Screw Piles Selector` and `Rock Anchor Selector`.
...
- Recalculate the bolt detailing status after normal input, selection changes and browser page restoration; restored inputs must not retain a stale PASS / FAIL state.
...
PFC catalogue merging must retain the checked direction and effective-property records from the shared hot-rolled directory while preserving the material strengths resolved from AS/NZS 3679.1. The raw PFC grade object must not overwrite the resolved `f_u`: use `440 MPa` for Grade 300 (300PLUS) and `480 MPa` for Grade 350, subject to the shared product-form material basis.
...
- Size the selected-family guide by annotation density rather than forcing every section into one canvas. On wide desktop, compact CHS/RHS/SHS/Round Bar guides may use an approximately 180 px visual column and 104 px image height; dimension-dense UB/UC guides may use an approximately 196 px column and 112 px image height. Keep the SVG viewBox clear of all dimension and axis labels, use `object-fit: contain`, and retain the shared compact/mobile fallback. The guide must not force an otherwise single-row four-metric summary onto another row.
...
- `f_y` and `f_u` may default from the selected material grade, manufacturer table or product standard, but keep them editable where project certificates, thickness ranges or product-specific values may govern. If `f_y` is overridden for a catalogue family, recalculate `k_f = A_e/A_g` from the active family geometry in accordance with AS 4100 Cl. 6.2.2 to Cl. 6.2.4 and update the applicable `alpha_b` table branch. If only `f_u` changes, retain the current `k_f`. Calculation steps must distinguish `Selected-grade catalogue value` from `Calculated for adopted f_y`.
...
- Enable family-dimension overrides only for CHS and Round Bar, where ideal circular geometry gives an axis-independent gross-property path. An active CHS override must calculate the circular effective diameter, `A_e` and `k_f` for the active `D`, `t` and `f_y`; Round Bar retains `A_e = A_g` and `k_f = 1.0`. Neither override may inherit `k_f` from the catalogue row used to initialise its dimensions. UB, UC, PFC, RHS, SHS and EA remain catalogue-only; Custom / Built-up is the verified-property route for non-catalogue members.
...
- Reconcile every supported selected-grade catalogue `k_f` against the independent AS 4100 effective-area calculation. Use the published rounded `k_f` for an unchanged selected grade only when the independent value agrees within `0.002`; otherwise fail closed. For an overridden `f_y` or active CHS geometry override, adopt the unrounded calculated value. Use flat-element limits `16` for hot-rolled outstands, `45` for hot-rolled elements supported along both longitudinal edges, `40` for cold-formed RHS/SHS supported elements, and `82` for CHS in accordance with AS 4100 Table 6.2.4. Retain the actual plate-element calculation in the folded trace rather than adding a routine `k_f` input.
...
- Capacity selectors must use the shared checked-design-row filter. A row is eligible only when the selected family has the required checked axis data and at least one checked grade record. Geometry-only rows remain discoverable in Section Properties and must not receive blank grades, placeholder capacities or inferred design values.
- Preserve the shared directory order in every dependent selector. For hollow sections this is descending primary size, then descending secondary size and thickness; downstream mapping must not re-sort the accepted array independently.
...
| `EA` | InfraBuild Tables 19 and 20 | Manufacturer load directions `A`, `B`, `C`, `D` | `Not evaluated` | All 46 checked catalogue designations enabled for direction-specific moment only |
...
Do not call a capacity selector a complete product directory. Each Beam family must consume every checked design-property row in the shared directory; geometry-only or otherwise unsupported rows remain visible in Section Properties and are excluded by the shared filter with a stated reason.
...
| 6 EA | Complete for all 46 accepted designations, including Table 19 principal-axis `I`, `Z`, `S`, Table 20 Load A / B / C / D `Ze` and direction-specific AS 4100 interval coordination | Product availability and current supply still require confirmation |
...
- Weld symbols are visual guides only. Visually check every displayed symbol against the applicable AS 1101.3:2026 source-PDF figure and show the exact figure number on the same card; do not rely on one undifferentiated source list below the legend. Label the legend `Schematic only, not to scale`.
- Do not place screenshots or copied Standards figures in the public repository unless the applicable reproduction permission has been recorded. Use minimal SVG linework that matches the cited symbol geometry and placement, and state that the Standards figure is not reproduced.
- For an arrow-side square-butt weld, one short line extends perpendicularly from the reference line towards the arrow side, consistent with AS 1101.3:2026 Fig. 5.1 and Appendix B Fig. B.2. A both-side square-butt weld uses one such line on each side; do not draw two parallel lines on the same side. A backing-run or backing-weld symbol placed above the reference line is an unfilled arch with its ends at the reference line and must remain opposite the applicable butt-weld symbol. Keep the complete-penetration-from-one-side supplementary symbol filled and opposite the basic weld symbol.
...
- Provide one optional `Footing projection, l_v` input measured from the loaded face to the pad edge. Use it only to screen the AS 3600 Cl. 12.1.1 footing proportion criterion `l_v/D < 1.5`; do not request overall footing length, loaded-area width or a second design direction. A blank value leaves non-flexural applicability `Not assessed` without blocking section capacities. A non-positive completed value is invalid. When the ratio is below 1.5, show a concise main-page review flag and state that an applicable Section 12 non-flexural design method is required. At or above 1.5, state only that the proportion criterion is not triggered; never imply that local D-regions are cleared. Put the ratio, boundary and full explanation in `Detailed section checks` and `Calculation steps`. Do not calculate struts, ties, nodes or anchorage. For a composite pad-on-pad strip, state that use of the total depth in this screen assumes effective composite action between the pours.
...
Geo70: label rock bearing as a conditional preliminary reference, not UCS-derived. Keep desktop table headings visible below the shell header (or at the top in the standalone entry). Clay shaft values appear once in the overview, with source/calculation basis folded below. Render approximate upper bounds in plain language, e.g. Up to about 4 kPa.

Geo68 table presentation: put units and conditions on a secondary 12 px line under the 13 px parameter heading. Stack paired labels above their values, top-align overview cells and retain the same numbers and source conditions. Reflow the Geo reference tables at 960 px to avoid compressed tablet columns; keep semantic headers accessible.

Geo67 expands the Geo66 first-look priorities without adding source ranges: show Clay and Sand unit weights and conditional shaft estimates, Gravel relative-density index and Rock point-load index. Geo69: Sand overview shaft values stay at explicitly labelled σ′_v = 100 kPa. The adjustable assumed-stress selector belongs in folded Pile shaft details and changes only that supplementary table. Preserve source gaps, method labels, clay shaft cap, and the distinction between rock test indices and foundation resistance.

Geo66 supersedes the uniform four-column overview: Clay leads with s_u, q_a and E_s; Sand with φ′, E_s and q_a; Gravel with φ′, E_s and γ; Rock with UCS and q_a. Keep material-relevant numerical foundation references visible after the overview: bored-clay shaft estimates, sand shaft stress benchmarks, gravel concrete-interface factors and lithology-specific rock-anchor bond. These are conditional references, not adopted resistance. Selected details remain directly after the overview and before the supplementary quick references. Do not add mandatory application modes or copy sand factors to gravel.

User-authorised Geo66 numerical extension: calculate approximate bored-clay shaft ranges from the existing s_u bands and Look Table 21.17 factors, retaining the 100 kPa cap and rounded-band qualification. Sand displays f_s at explicit σ′_v benchmarks of 50/100/200 kPa, separately for bored, driven low-displacement and driven displacement piles. Benchmarks are assumed stress scenarios, not source material ranges, allowable stresses or total capacities. Label their basis visibly; do not derive depth from a hidden groundwater assumption.

Geo65 formatting: use canonical 12/13/15/22 px typography; aligned 46 px desktop and 44 px phone selects with 16 px phone input text. Display range-type labels separately above values. Standalone and integrated entries share the title and disclosure hierarchy. Keep numerical data and source conditions unchanged.
...
Geo62 (21 September 2026): detailed values use Reference value / range; one material-coverage note; gross/net qualification stays in soil-bearing evidence; AS 1726 check labels state classification-table scope and pending currency. Current implementation plan is consolidated; older records are historical. Local / Unreleased.

Geo61 / 20260921geo61 (21 September 2026): material lookup scope beside tabs; family navigation removed from refinements; clay subtype and rock lithology retained; independent-range note visible below each overview; consistent Intact rock strength; source-qualified fines wording; Pile shaft resistance with explicitly dimensionless factors. Numeric dataset unchanged; Local / Unreleased. The material tabs are the sole material-navigation control; hidden family state is internal only. USCS is a separate permeability refinement, not an automatic AS 1726 equivalence.

Geo47 scoped follow-up: `geo-parameters/GEO47_FOLLOWUP.md` records source-card clarification and keyboard acceptance; cache `20260905geo47`. Government-hosted project reports and drawings are site-specific examples, not government generic design recommendations. Identify their action basis and pending source rechecks without altering the established generic comparison ranges. Native zoom and assistive-technology speech remain separate manual gates.

Geo46 evidence addendum (5 September 2026): `geo-parameters/GEO46_ACCEPTANCE.md` is the current scoped author acceptance record, indexed by `PREPUBLICATION_REVIEW.md`. Keep material applicability visible: sand DCP bands must not transfer to gravel; the selected sand modulus source is medium/coarse sand; permeability carries its well-compacted basis; rock weight is dry; PI bands are source-specific rather than AS plasticity classes. Retain numerical ranges only within their source conditions. The 50-file test suite, 608 controlled selections and 127 local-browser assertions are author evidence, not independent or publication approval. Native zoom, assistive-technology acceptance, source currency and publication decisions remain separate gates.

`Ground Parameters` is a professional lightweight query reference for standard ground-description terminology and commonly requested foundation parameter ranges. It is not a geotechnical report interpreter, a ground model, a parameter-selection engine or a foundation design calculator.

Publication record:

- `Publication class`: `Unreleased`.
- `Public result`: none; the page is excluded from `public-build-manifest.json`.
- `Protected content`: source PDFs, extracted source tables, rendered source pages and review notes remain outside the public artifact.
- `Rights-review boundary`: common geotechnical parameter names, symbols, units and independently written engineering explanations may remain in the page. Review source-specific wording, figures, copied table structures and any substantial selection or arrangement item by item; do not impose a blanket publication block merely because the page cites AS 1726 or an interpretive handbook.
- `Deployment target`: local main-app tab at `index.html#geo`, with `geo-parameters/index.html` retained as an isolated review harness. Integration does not authorize publication.
- `Public-build boundary`: all main-app Geo blocks use matched `UNRELEASED:geo` markers. `scripts/build-public-site.js` strips them from allowlisted HTML, CSS and JavaScript; `geo-parameters/` remains outside `public-build-manifest.json`.

The Geo49 reading workflow is a source-backed lookup for structural engineers who may not already know the geotechnical symbols. The design logic is ground model and conditions -> bearing/strength and settlement -> stability/interface or pile/anchor needs -> project verification. These checks interact in real design; the page exposes references and does not perform them. The reader does not need to select an application mode or enter loads/geometry.

1. Geo60 presents material sub-tabs Clay, Sand, Gravel and Rock under Ground Parameters. These are lookup materials, not a complete classification system. Show only the active material comparison table. Clay retains cohesive-reference coverage; Sand and Gravel retain granular coverage. Each state row uses the material-specific Geo66 priority columns defined above; other parameters remain in selected details. Build these values from the same guide-query model as detailed results; retain units, dry/saturated labels and critical source conditions. Start with no selected state or adopted default value. Preserve the explicit silt and special-ground scope boundaries.
2. Selecting a state opens one shared detailed parameter table directly below the active material comparison, within its labelled tabpanel. Switching material clears the selected state, old result rows and incompatible refinements. Use accessible tablist/tab/tabpanel semantics, roving focus and ArrowLeft/ArrowRight/Home/End navigation. Default to the Clay overview without selecting a ground state. All source-qualified parameters appear here; no separate Full parameter reference. The overview repeats only the material-specific key parameters and short foundation references, not a second full inventory. Group additional classification, tests and project requirements within the same table. Focus the selected heading and announce state changes.
3. Refine selected ground precedes the table and starts collapsed. Keep compatible material, state, weathering, plasticity and USCS controls. Family changes clear incompatible refinements. Missing gravel ranges must never inherit sand values.
4. Each parameter carries its critical conditions and an inline Source and basis disclosure where further detail is needed. Primary permeability requires matching USCS; retain the broader family envelope only inside its own source note. Every original source-parameter row must be represented exactly once in the unified model, with its source conditions retained.
5. Parameter-specific source tables follow their relevant rows: bearing, deformation, pile shaft friction, concrete interface and anchor bond. Keep shaft and anchor reference tables separate. Sand shaft factors remain dimensionless and installation-specific; the separately labelled Geo66 benchmark table applies the effective-stress relationship. No allowable stress or total capacity is calculated. Additional project-adopted parameters remain clearly distinguished from generic source ranges.
6. Keep the project checklist and source registry folded below the single table. Preserve numeric source data, canonical typography, semantic table structure and the Unreleased boundary. This user-authorised workflow supersedes older multi-table presentation descriptions below.


Geo43 state/copy refinement: activating the expanded state again closes its details; reopening preserves compatible refinements. Family changes still clear incompatible classifications. Rock headings show weathering or explicitly state that it is not selected. For residual soil / extremely weathered material, remove inherited UCS and point-load classification bands, hide the inapplicable rock-strength control and show soil-strength/stiffness project requirements without guessing a soil class. Retain only an exact applicable lithology/weathering unit-weight row, labelled material unit weight. A deliberate rock-strength row selection exits the soil-like weathering branch. Keep material-specific notes and formatted subscripts; no numerical source values or release status are promoted.

Foundation-question contents:

Geo44b evidence addendum: the conditional ultimate example cites FHWA GEC 6 (2002), Eq. (5-4) and Table 5-1, separately from AS 1726 Table 11 classification bands. State level ground, centred vertical load and zero-surcharge gross/net equivalence for the surface-strip example. Round displayed limits to 1 kPa and label them approximate. Keep clipped mobile table headers in the accessibility tree and source-link text contrast at least 4.5:1 in the tested default state. `geo-parameters/PREPUBLICATION_REVIEW.md` records the current author evidence and unclosed independent/accessibility/publication gates; it does not confer publication approval.
- Bearing distinguishes presumed/preliminary allowable q_a, derived conditional ultimate q_ult screening and adopted project resistance. Preserve the complete six-state clay q_ult screening table, not just a warning. State the idealised fully undrained homogeneous surface strip footing and q_ult = 5.14s_u beside it; retain approximate rounded bounds and no settlement/shape/embedment/layering allowance. No universal factor converts allowable to ultimate. Sand/gravel/other ultimate cases require geometry and a project strength/groundwater model.
- Keep the complete Look Table 22.1 RQD allowable-bearing reference in Bearing, reached by an explicit link beside the rock overview. Show the lesser-applicable-limit instruction, UCS/concrete-stress qualifications, defect/weathering checks and the source exclusion of detailed design. For AS 1726 residual soil or extremely weathered material with soil properties, suppress the selected rock-bearing reference and direct the user to soil description/project assessment.
- Settlement refers to selected E and ν without repeating their values. Its compact additional-input table explains consolidation, time effects and subgrade reaction with project conditions, not repeated missing-value cells. Define total movement, support-to-support difference and relative rotation concisely; no universal permissible movement or fictitious range is added.
- Sliding retains the seven FHWA mass-concrete interface rows, their ultimate factor/angle basis and contact qualifications. Internal soil friction, base-interface friction and undrained adhesion are distinct; no combined resistance is calculated.
- Piles and anchors retain the five clay-factor, four sand-factor and six ultimate rock-bond rows. Explain side/toe/fixed-length transfer before the tables. Dimensionless factors are not unit shaft stress; pile base pressure remains project-specific without a verified general source.

Copy and data policy: one page-level reference warning; full name, units, range nature and necessary applicability stay beside values. Detailed source clauses and explanations are folded. Replace terse missing-row messages in the primary view with No verified range for this selection and a useful project/refinement next step. Keep columns independent, avoid selecting midpoints and retain all prior verified source values. The reference PDF is not numerical authority. No source status or public entitlement is promoted by this reorganisation.

Use common Australian geotechnical notation with formatted subscripts: s_u, I_D, I_s(50), γ_d, γ_sat, E_s, E_rm, m_v, C_c, C_r, c_v, C_α, q_a, q_ult and k_v, with quantity/unit context where symbols overlap. For piling retain f_m,s and f_b; source aliases q_s / q_b may be explained in detail. Render c_a = αs_u and disclose Look's original notation rather than confusing C_u with coefficient of uniformity. Define K_s in K_s tan δ as the pile-interface earth-pressure coefficient, not modulus of subgrade reaction.

Source hierarchy and display status:

- FHWA GEC 6, *Shallow Foundations* (2002), Table 5-15 and Sections 5.3–5.4 support the base-interface comparison and settlement notes. These are historical US references, not Australian normative design parameters; source-row checks do not establish project applicability.
- AS 1726:2017 Tables 11, 12, 19 and 20 control cohesive-soil consistency, non-cohesive-soil relative density, rock material strength and weathering terminology. These rows are `Standard classification` and `Checked` only for exact terminology and bands.
- Burt G. Look, *Handbook of Geotechnical Investigation and Design Tables*, 2nd edition, supplies typical unit-weight, SPT/DCP indications, effective-strength, permeability, foundation-modulus, Poisson-ratio and bearing references. Pile-interface and anchor-bond rows are available in a separate folded comparison; measured-DCP bearing calculations remain archived. All such rows are `Interpretive` and require exact-row and project-applicability review.
- Application standards are routing references only: use the project-nominated editions of AS 2159 for piling, AS 5100.3 or AS 2870 where applicable, AS 4678 for retaining structures, and the nominated ground-anchor standard. Do not reproduce or imply a design value merely by naming an application standard.
- `Reference parameters for interpretation of borehole logs 1` is an internal practical reference only. It may guide the content inventory, but it is not a standard or formal design document and its unverified numerical rows must not control page output.
- Government portals such as QLD GeoResGlobe and equivalent state borehole systems are discovery routes for logs, tests and reports. Regional geology, lithology or a mapped unit must not be converted directly into a design parameter.
- Public geotechnical reports may be shown as separately labelled site-specific spot checks. Do not pool a small report sample into a national range, and do not let a project example control the generic lookup.

Keep these engineering boundaries explicit:

- A classification band is not automatically a characteristic or design parameter.
- Common report parameters that cannot be inferred from a material description must remain `Project report value required`. These include stratum levels/thicknesses, origin and variability, the adopted ground model, characteristic/moderately-conservative/design parameter sets, bulk/natural unit weight, water content, specific gravity, void ratio/porosity, Atterberg limits, shrink-swell/reactivity, collapsibility/dispersivity/slaking where relevant, measured field-test records and corrections, groundwater level/variation, consolidation parameters, OCR/preconsolidation stress, earth-pressure coefficients, liquefaction/cyclic-softening inputs and chemical/aggressivity or acid-sulfate indicators.
- The project-only inventory also identifies standardized SPT values `N_60 / (N_1)_60`, CPT `q_c / q_t / f_s / R_f / u_2`, sensitivity and remoulded/residual strength, permeability anisotropy `k_h / k_v`, conditional dynamic inputs `V_s / G_0 / G_max / damping`, rock-defect shear parameters `c′_j / φ′_j`, shallow-foundation sliding/uplift/global-stability/scour checks and pile `t-z / q-z / p-y` models. These remain conditional needs, not default values.
- Foundation-design needs remain available in the folded Before design checklist and selected full parameter list; keep project requirements distinct from numerical references. No separate foundation-application input mode is introduced.
- Before selection, show the complete comparison but do not highlight a default state or populate a selected bearing result. After the user selects a matching clay or sand state, the result may show Look Table 21.4 presumed bearing pressure `q_a`. Label it `Presumed` and `Preliminary · 25 mm`; disclose that it assumes natural material and requires the source groundwater, footing-width and strip-footing modifiers. Do not transfer the sand table to gravel. For cohesive soil, the detailed inventory may also show an undrained `q_ult` screen from `q_ult = 5.14s_u` using the selected AS 1726 consistency-band limits for a fully undrained surface strip footing. This screen has no embedment, shape, layering or settlement allowance and is not project resistance. Other ultimate-bearing cases remain dependent on project geometry and parameters.
- Reserved for a future measured-test reference, currently archived: a detailed selected result may additionally repeat the cited DCP-based allowable bearing-pressure range `q_a` only after the user selects a measured DCP band. Because the cited row states `FS = 3`, the page may also show the transparent screening equivalent `q_ult,screen = 3q_a`. Both remain `Interpretive`; the latter is not an independent ultimate-capacity calculation, characteristic resistance or project design resistance, and must retain the same gross/net basis as the cited allowable row. Keep project-only design bearing resistance, settlement-limited pressure, `k_v`, base-interface and founding-inspection rows separate. DCP must default to `Not selected`; no measured DCP result may be inferred from the selected soil description.
- Pile design requires project unit shaft resistance/adhesion in compression and uplift, ultimate/serviceability end bearing, lateral response, downdrag, group/settlement and construction/testing requirements. An `alpha` or `ks tan delta` correlation is not itself a unit shaft resistance.
- DCP, SPT and field correlations require the matching test equipment, procedure, corrections, deposit condition and local applicability; one test result must not silently prove a ground class. For clean medium sands distinguish uncorrected field SPT N from the corrected source value; do not transfer the sand row to gravel.
- AS 1726 Table 20 weathering descriptions must preserve the current material-based distinction: highly weathered rock has significantly changed strength, moderately weathered rock has little or no strength change, and distinctly weathered is used only where HW and MW cannot practicably be distinguished. Do not substitute older percentage-decomposition wording.
- Look Table 7.9 values are typical peak effective strengths. Keep them visibly distinct from critical-state, softened, remoulded and residual strengths, and state that long-term effective cohesion may be lost.
- Look Table 7.3 provides representative dry and saturated unit weights. Natural/bulk unit weight remains a project value, saturated unit weight applies below groundwater and in the capillary fringe, and buoyant unit weight must be derived separately.
- Where the basic logic states `γ′ = γsat - 9.81 kN/m³`, show the transparent source relation only. Do not relabel the representative dry value as natural/bulk unit weight or silently calculate an adopted groundwater design value.
- Soil/rock material parameters and pile/anchor interface parameters are separate concepts.
- The folded pile reference shows the cited α and K_s tan δ estimation factors for the source installation and soil classes. Preserve the driven-displacement limitation, low-displacement reduction note and overlapping very-stiff clay bands. Do not infer additional soil classes or calculate shaft resistance, base resistance, capacity or settlement.
- Look Table 21.17 places `Very stiff` clay in both adjacent driven-pile alpha bands. The page must show `0.25–0.75` with `Source bands overlap · project selection required`; it must not silently resolve the overlap to one factor.
- Rock output must keep intact-rock classification separate from project rock-mass evidence. List measured UCS / point-load correlation, TCR / RQD, fracture frequency, controlling defect-set characteristics and the nominated GSI / RMR / Q or other project rock-mass model where required; do not synthesize these from AS 1726 weathering or strength classes.
- Rock output may reproduce Look Table 22.2 strength-class allowable-bearing first approximations and the Look Table 22.1 RQD bearing reference. Label both `Preliminary`; show the table instruction to use the lesser of the applicable table value, UCS and allowable concrete stress; and state that the RQD method is not appropriate for detailed design. Rock bearing remains dependent on core logging, defects, weathering, geometry and project verification.
- The folded anchor reference shows separate Table 20.22 rock-type rows with ultimate grout–rock bond stress in kPa. State the intact-rock assumption and the need for project-justified reductions for fractured/weathered rock; retain the source FS as context without calculating a design value. The table is a general comparison, independent of the selected rock state. Do not merge soft/hard rock rows or calculate fixed length, anchor capacity or utilisation.
- When optional plasticity or USCS classification is absent, a broad source-table material-family envelope may be displayed as `Interpretive` to keep the quick reference useful. It must be labelled as an envelope and must narrow only after the user selects a verified classification.
- Shallow-foundation output may reproduce the cited presumed-bearing and DCP allowable-bearing ranges with their stated limitations, plus the explicitly derived screening ultimate values above. It must not recommend a design bearing pressure or claim to assess settlement.
- Never output a synthesized `recommended`, `adopt`, `design`, `best estimate`, `PASS` or `FAIL` result from the reference ranges.

Required visible adoption warning: the project geotechnical report must confirm the test method, drainage and stress basis, groundwater, variability, characteristic/design-value selection, foundation geometry, installation effects, settlement, group effects, rock mass defects, durability and verification testing.

Geo49 places default-collapsed full-width source disclosures beside their corresponding queried parameters. No separate question section or design mode is used. Uses appear below parameter names; conditions remain next to values. The initial five-column ground comparison requires no input.
- Geo45 deduplication: retain selected source-backed modulus and Poisson-ratio ranges in ground details, not a second Settlement table. Bearing refers to selected allowable ranges without repeating them; the complete conditional ultimate and RQD tables remain. Identify project-adopted allowable pressure separately from presumed reference pressure. Distinguish permeability k_v (m/s) from vertical subgrade reaction modulus k_v (kN/m³) by full name and units. Separate rock-mass modulus and Poisson ratio. Show the hydrostatic submerged-weight relation beside soil unit weight without calculating an adopted value. The full folded inventory remains available for source traceability.
- Base interface presents the seven mass-concrete rows from FHWA GEC 6 (2002) Table 5-15 with ultimate friction coefficient and interface angle. Identify this as a US historical estimate and preserve the source material descriptions rather than silently mapping them to AS 1726 classes. Adhesion is a separate undrained project input; do not sum it with drained friction or compute sliding resistance. Keep coefficient and angle as separately rounded source values.
- These are additional evidence contracts; neither US bridge-specific acceptance limits nor safety factors are adopted as Australian requirements. Earth-pressure coefficients and advanced foundation calculations remain outside this expansion.

Reading rules: preserve visible numerical ranges with concise qualifications. State that columns are independent references, not a matched parameter set. Distinguish unit weight from density and representative dry/saturated pairs from intervals. Identify the unresolved gross/net basis of presumed bearing; the Table 21.4 starred groundwater modifiers apply to sand, not automatically to clay. Mark rounded ultimate screen limits as approximate. Keep special-ground exclusions in one concise line.

Implementation and validation:

- Keep data in `geo-parameters/geo-parameters-data.js`, pure selection/presentation logic in `geo-parameters/geo-parameters.js`, shared DOM binding in `geo-parameters/app.js`, main-tab base styles in `geo-parameters/tab.css`, shared Geo42 workflow styles in `geo-parameters/guide.css`, and the standalone review harness in `geo-parameters/index.html`. `geo-parameters/CONTENT_PLAN.md` is the implementation/source matrix, not a second governing outline.
- Keep all visible text English-first and use the exact accepted page name `Ground Parameters`.
- Test every reproduced source row, controlled state, unavailable combination, terminology label and public-build exclusion.
- Apply the full Section 15.19 audit protocol; this tab-specific list is a minimum, not an exemption. Record source row, units, conditions, source-to-AS terminology mapping and displayed value for each active lookup. Give soil presumed bearing, rock bearing/RQD and the derived undrained ultimate screen separate contract IDs.
- Browser review covers the unselected default, selected cohesive, sand, gravel and rock states, family changes, unavailable rows and collapsed/expanded details. Use the global viewport set, keyboard operation, visible focus, meaningful accessible names and status announcements, and 200% browser zoom/reflow. Keep one current screenshot-linked audit row per bounded card in VISUAL_PAGE_AUDIT.md.
- Record independent review separately from author source checks and regression results. Pending reviewer acceptance must remain visible in the evidence record; it does not disappear because the page is a reference aid.
- Maintain the current Geo evidence matrix in REFERENCE_TRACEABILITY.md, including source currency, item-level rights review, outstanding findings, exact revision, tests and visual evidence. Public-beta approval and promotion to Checked are separate decisions.
- Within the selected parameter table, the default-collapsed pile-factor and anchor-bond disclosure shows source-row comparisons without inputs: clay adhesion factors, sand pile-interface factors and intact-rock ultimate grout–rock bond stress. Preserve source installation classes, overlapping clay bands, exact rock-type rows and numerical units. Factors are dimensionless; they are not unit shaft resistance. The disclosure never selects a design factor or calculates pile/anchor resistance. Its source registry and rights review remain separate from the retired application prototype.
- The retired Shallow/Pile/Anchor prototype is archived in archive/geo-application-prototype.md and is not loaded by either page. The active query accepts ground-description input only and rejects obsolete application modes. Historical application-specific rules above describe conditions for any future reintroduction, not active interface controls. Preserve the common bearing ranges and project-input checklist when simplifying runtime code.
...
2. `Check selection`: design check, bar size and, for development only, bar installation;
3. `Design conditions`: calculation-branch selections;
4. `Inputs`: editable physical values;
5. calculated bases, including `Calculated cd`;
6. path-specific primary result;
7. optional engineering adjustments;
8. calculation details and references / limitations.

For the lap path, expose only the selections that commonly change the calculation branch: `Member geometry` and `Casting position`. Place them under `Design conditions` immediately after the check and bar size. Keep the preselected but editable `Tension-tie classification`, `Lap arrangement`, `Calculation method` and `Material condition` under one collapsed `Additional conditions` control. Its summary must state the active values so no assumption is hidden. The default set is `Not a tension-tie`, `Contact lap`, `Basic method`, and `Normal-weight / uncoated`. Do not preselect a favourable reduction, confinement contribution, top-cast condition or reduced project stress.

For development at termination, group the concrete and bar conditions as two engineering rows on desktop: `Member geometry` with `Casting position`, followed by `Concrete strength` with `Material condition`. Use a single-column stack on phone. Do not force the four fields into one row or truncate the calculation-branch labels to make them fit.

Use the same two-column rhythm for development geometry. Place `Clear cover` with `Clear spacing` in the first row and show the conditional `End / side distance, c1` as an additional editable input. Do not style `cd` as an input. Place it immediately below its source inputs in a neutral, full-width `Calculated basis` strip. Apply the same derived-output treatment to the lap path. Stack editable fields in one column on phone while keeping the calculated strip visually distinct.
...
Use concise professional engineering language without replacing AS 3600 terminology. State each fact once. Prefer noun headings and physical quantities over interface instructions: `Check selection`, `Design conditions`, `Inputs`, `Calculated cd`, `Bar stress`, `Geometry`, `Existing concrete`, and `Concrete and reinforcement`. Do not add `Choose`, `Enter`, `Use` or `These selections` helper text where the control label is self-explanatory. Do not repeat `AS 3600` in a child field when the parent route already establishes that reference basis. Show a limitation only at the result or when the user selects the condition that triggers it. A main-path label states the physical item first and retains the formal symbol where it assists review, for example `Clear spacing between bars, a` and `Calculated cd`. Use `Wide member - slab, wall or flange` and `Narrow member - beam web or column core` directly in the selector. In the optional `Refined development method`, keep `n_f`, `n_bs`, `Sigma A_tr`, `k_4`, `k_5` and `rho_p` visible for traceability. Do not remove a qualification, confirmation or fail-closed state merely to make the page simpler.

Use four restrained typography levels within the tab: the tool title; the active path title; input-group headings; and field labels. Give `Check selection`, `Design conditions`, `Inputs`, the active development basis, concrete conditions and `Geometry` the same 14 px strong heading treatment with one short accent rule. Keep field labels subordinate at the shared form-label weight. Show the active path as one plain 16 px descriptive title without an adjacent uppercase tag or locator badge. Do not apply the input-heading treatment to the neutral `Calculated cd` strip, result cards or optional disclosure summaries; their role must remain distinguishable without additional decorative headings.

Use `Design check` as the primary classification: `Lap splice` or `Development at termination`. Where a development check is active, show `Bar installation` as the secondary route: `Cast-in reinforcement` or `Post-installed reinforcement (PIR)`. Show `Bar size` for every length-producing check. Use `Calculation method` for the Basic / Refined selector so it is not confused with the governing AS 3600 development basis. Do not show a project-scenario selector: general members, foundation extensions and tower/monopole pedestal modifications do not change the AS 3600 expression and remain project context rather than a calculation branch. For cast-in development at termination, separately select the `Anchorage method`: `Straight development`, `Standard hook`, or `Standard cog`; a hook or cog is an anchorage method and never a lap reduction. For PIR, the selected full-yield or qualified project steel-stress basis calculates an `AS 3600 reference depth`, not a preliminary recommendation or installation depth. Product selection and the current qualified project report remain external to this quick-reference page.
...
| Development at termination | Cast-in / straight | `DEVELOPMENT REFERENCE LENGTH` |
...
Place primary results immediately after applicable inputs and calculated bases. Keep lap and development at termination as independent blocks. The lap block shows `Adopted lap length` and its governing basis. Cast-in straight development uses `Development reference length`, not `Required development length`, because the page does not complete member detailing or the project load path. The PIR block gives primary emphasis to `AS 3600 reference depth` and states `Not an installation depth - product design required`. Use the concise basis states `FULL YIELD`, `PROJECT STRESS` or `INPUT REQUIRED`; retain longer text only for an unresolved qualification such as Refined reconciliation or hook/cog detailing. Do not show available embedment, a product selector, a report form or a site-fit comparison. For a cast-in starter, show the selected straight-development or hook/cog reference as the primary result.
...
Keep the visible page order as `Input data`, `Derived properties`, `Capacity results`, then folded calculation details and sources / limitations. Within `Input data`, use three peer groups only: `Geometry`, `Profile segment schedule`, and `Material and resistance basis`. Do not retain duplicate stage/group pairs such as `Section definition` plus `Section form`, or `Material and fabrication` plus `Material properties`. Use the canonical shared typography tokens: `--fs-md` for stage and result-disclosure headings, `--fs-sm` for input-group headings, labels, values and compact result labels, and `--fs-xs` for helper, source and limitation text. Stage headings do not need explanatory subtitles when the child groups already define their content. Concise scope notes may identify global section-form, material-basis and fabrication-category inputs as common to all segments. Do not use Monopole-specific 17 px or 18 px heading sizes.

Use background colour only where it communicates hierarchy, function or state: a restrained stage-heading tint with a 3 px left rule, manual/automatic input state, schedule table headers, the derived-property summary, an expanded capacity disclosure and warning/unavailable states. Ordinary Geometry or Material groups remain unfilled. Do not add letter or number locators, stage-heading shadows, or internal decorative cards. The segment schedule may retain a restrained frame because it is a distinct editable table.
...
Use `Nominal wall thickness, t_nom` as the primary thickness input. By default, `t_d = t_nom`. The optional `Design thickness override, t_d` workflow permits a smaller positive project input, with `0 < t_d <= t_nom`. When active, identify it as `User override`. While the override is disabled, keep each hidden `t_d` value synchronised with the current segment `t_nom` so re-enabling cannot restore stale thickness data. The AS/NZS 3678 Table 8 lookup and theoretical mass use `t_nom`; section properties and bending resistance use `t_d`. A `t_d` override does not require manual `f_y`. Corrosion allowance and measured remaining thickness are not otherwise calculated by the page.
...
| Yield stress source and grade | source selection | `AS/NZS 3678 plate lookup` or `Manual yield stress`; the initial Austube CHS example adopts editable `f_y = 350 MPa`; the KOP-1230 polygon verification case adopts editable `f_y = 355 MPa` from its E355BR designation |
...
Within each segment, interpolate its outside dimension using that segment's own local coordinate. Do not interpolate one global pole taper through an overlap. At a slip joint, the outer upper shell and inner lower shell retain their own dimensions, thicknesses, materials, section properties and capacities. At both overlap endpoints, require the upper shell nominal inside dimension to be at least the lower shell nominal outside dimension. This is a necessary nominal nesting screen only; tolerances, required clearance, seam geometry, ovalisation and installation force remain outside scope.

Report the governing minimum nominal clearance as geometry information only. Do not apply a numerical `PASS / FAIL` clearance limit unless the project supplies a governed fit-up tolerance. Label positive clearance for project fit-up review so a large geometrically valid gap cannot be mistaken for an accepted joint.
...
- `Derived properties`: one compact read-only summary for mass, self-weight and centre of gravity;
...
Place the compact `Derived properties` summary between material inputs and capacity results. It reports mass, self-weight, assembled centre of gravity and installed height; it is not a capacity result. Place prescribed slip-overlap screening after capacity results, followed by calculation details, sources and limitations. Do not use a generic `Results` heading or a result-card grid.
Keep both capacity disclosures collapsed by default so their base summaries can be compared directly. Each disclosure uses the common 0.5 m station basis. In the expanded moment profile, identify the minimum evaluated station without describing it as a continuous numerical optimisation. Hide the prescribed slip-overlap section when no positive overlap is entered.
...
Any visible helper or result text that states `r_i/t_nom` must be generated from the live input. A default value may appear in the initial control and source example, but must not remain hard-coded in recalculated page text.
